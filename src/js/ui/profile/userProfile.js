...
export async function renderUserProfile() {
  ...
  if (!username) {
    document.getElementById('authorContainer').innerHTML = '<p>Error: No username provided.</p>';
    return;
  }
  ...
  try {
    ...
    document.getElementById('authorAvatar').src = avatar?.url || 'https://via.placeholder.com/150';
    document.getElementById('authorName').textContent = name || 'Unknown User';
    document.getElementById('authorBio').textContent = bio || 'No bio available';

    document.getElementById('postCount').textContent = _count?.posts || 0;
    document.getElementById('followerCount').textContent = _count?.followers || 0;
    document.getElementById('followingCount').textContent = _count?.following || 0;
...
function setupFollowButton(username, followers) {
  ...
  followButton.addEventListener('click', async () => {
    ...
    followButton.textContent = isCurrentlyFollowing ? 'Unfollowing...' : 'Following...';

    try {
      const newFollowStatus = await onFollowToggle(username, isCurrentlyFollowing);
